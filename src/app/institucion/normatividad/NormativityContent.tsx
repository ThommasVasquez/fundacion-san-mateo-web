          ...
          <div className="max-w-xl">
            <div className="flex items-center gap-3 mb-6">
              ...
              <span className="text-[10px] font-black tracking-[0.4em] text-fsm-red uppercase">
                {getC('norm_hero_subtitle', 'Marco Legal')}
              </span>
            </div>
            <h1 
              className="text-5xl md:text-7xl font-black text-fsm-blue leading-none mb-8"
              dangerouslySetInnerHTML={{ __html: getC('norm_hero_title', 'TRANSPARENCIA <br /> <span class="text-fsm-blue-light">NORMATIVA</span>') }}
            />
            <p className="text-lg text-gray-700 font-medium leading-relaxed">
              {getC('norm_hero_desc', 'Consulte nuestra base documental, resoluciones de aprobación y manuales institucionales que garantizan nuestra excelencia académica.')}
            </p>
          </div>
        </div>
        <div className="lg:w-1/2 relative min-h-[300px] lg:min-h-full overflow-hidden bg-fsm-blue">
          <Image 
            src={getC('norm_hero_image', '/img/banner12.jpg')} 
            alt="Normatividad" 
            fill 
             ...
             <div className="p-12 md:p-16 bg-fsm-blue rounded-[4rem] text-center relative overflow-hidden group shadow-premium">
                ...
                <div className="relative z-10 max-w-2xl mx-auto space-y-8">
                   <h4 
                    ...
                    dangerouslySetInnerHTML={{ __html: getC('norm_cta_title', '¿Requiere Consultar <br /> Más Información?') }}
                   />
                   <p className="text-white/60 text-lg font-medium leading-relaxed">
                     {getC('norm_cta_desc', 'Nuestro archivo institucional está disponible para consulta en la sede administrativa para toda la comunidad académica.')}
                   </p>
                   <Link 
                   ...
                   >
                     {getC('norm_cta_button', 'HABLAR CON SECRETARÍA')} <ArrowRight size={18} />
                   </Link>
                </div>
             </div>
          </div>
