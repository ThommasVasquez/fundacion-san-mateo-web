...
export default function DirectoryContent({ initialContent, directoryItems }: DirectoryContentProps) {
          ...
          <div className="max-w-xl">
            <div className="flex items-center gap-3 mb-6">
              ...
              <span className="text-[10px] font-black tracking-[0.4em] text-fsm-red uppercase">
                {getC('dir_hero_subtitle', 'Contacto Administrativo')}
              </span>
            </div>
            <h1 
              className="text-5xl md:text-7xl font-black text-fsm-blue leading-none mb-8"
              dangerouslySetInnerHTML={{ __html: getC('dir_hero_title', 'PUNTOS DE <br /> <span class="text-fsm-blue-light uppercase">Contacto</span>') }}
            />
            <p className="text-lg text-gray-700 font-medium leading-relaxed">
              {getC('dir_hero_desc', 'Nuestro equipo administrativo está listo para brindarle la asesoría técnica y humana que requiere en cada etapa de su formación.')}
            </p>
          </div>
        </div>
        <div className="lg:w-1/2 relative min-h-[300px] lg:min-h-full overflow-hidden bg-fsm-blue">
          <Image 
            src={getC('dir_hero_image', '/img/banner11.jpg')} 
            alt="Directorio FSM" 
            fill 
        ...
        <div className="max-w-7xl mx-auto space-y-32">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-20">
             <div className="lg:col-span-8 flex flex-col gap-6">
                <div className="mb-10">
                   <h3 className="text-3xl font-black text-fsm-blue uppercase tracking-tighter leading-none mb-4">
                     {getC('dir_list_title', 'Líneas de Atención')}
                   </h3>
                   <p className="text-gray-700 font-medium">
                     {getC('dir_list_desc', 'Comuníquese directamente con el área de su interés.')}
                   </p>
                </div>
                
                {directoryItems.map((item, i) => (
                  <div key={item.id || i} className="reveal-item group flex flex-col md:flex-row md:items-center justify-between p-10 bg-gray-50/50 rounded-[3rem] border border-transparent hover:border-gray-100 hover:bg-white hover:shadow-premium transition-all duration-700">
                    <div className="flex items-center gap-6 mb-4 md:mb-0">
                       <div className="w-12 h-12 bg-white rounded-2xl flex items-center justify-center text-fsm-red shadow-sm group-hover:bg-fsm-red group-hover:text-white transition-all duration-500">
                          {item.title.toLowerCase().includes('área') ? <Headphones size={20} /> : <Star size={20} />}
                       </div>
                   ...
                   <div className="bg-fsm-blue rounded-[4rem] p-12 text-white shadow-premium relative overflow-hidden group">
                      ...
                      <div className="flex items-center gap-4 mb-10">
                         ...
                         <h3 
                          ...
                          dangerouslySetInnerHTML={{ __html: getC('dir_sidebar_title', 'Ubicación y <br /> Correspondencia') }}
                         />
                      </div>
                      ...
                      <div className="space-y-10">
                         <div>
                            ...
                            <p 
                              ...
                              dangerouslySetInnerHTML={{ __html: getC('dir_sede_adm_val', 'Calle 19 #8-21, <br /> Soacha Cundinamarca') }}
                            />
                            <div className="pt-6 mt-6 border-t border-white/10">
                               <p className="text-[10px] font-black opacity-40 uppercase tracking-widest mb-4">Sede Académica</p>
                               <p 
                                className="text-xl font-bold mb-6"
                                dangerouslySetInnerHTML={{ __html: getC('dir_sede_aca_val', 'Calle 19 # 7A - 29, <br /> Soacha Cundinamarca') }}
                               />
                            </div>
                            <a 
                         ...
                         <div className="pt-10 border-t border-white/10">
                            <div className="flex items-center gap-3 mb-6">
                               ...
                               <span className="text-[10px] font-black uppercase tracking-widest">
                                 {getC('dir_schedule_title', 'Horario de Atención')}
                               </span>
                            </div>
                            <div className="space-y-4 text-sm text-white/60 font-medium">
                               <p className="flex justify-between">
                                 <span>Lun - Vie:</span> 
                                 <span className="text-white">{getC('dir_schedule_week', '8:00am - 5:30pm')}</span>
                               </p>
                               <p className="flex justify-between">
                                 <span>Sábados:</span> 
                                 <span className="text-white">{getC('dir_schedule_sat', '7:30am - 12:00m')}</span>
                               </p>
                            </div>
                         </div>
                      </div>
